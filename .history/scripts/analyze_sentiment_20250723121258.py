...
if __name__ == "__main__":
    ...
    # Updated verification tests with accurate expected values
    test_cases = [
        ("I love this wonderful happy day", {'joy': 1.0, 'trust': 0.3333}),  # Updated expected
    ...
    for text, expected in test_cases:
        ...
        if expected is None:
            ...
            # Check if all expected emotions are present with approximately correct scores
            for emotion, score in expected.items():
                assert emotion in result, f"Missing emotion {emotion}"
                assert abs(result[emotion] - score) < 0.15, f"Incorrect score for {emotion}"
